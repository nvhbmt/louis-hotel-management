...
CREATE PROCEDURE sp_ThemCTHoaDonDichVu @maHD NVARCHAR(10),
...
CREATE PROCEDURE sp_CapNhatCTHoaDonDichVu @maHD NVARCHAR(10),
                                          @maDV NVARCHAR(10),
                                          @soLuong INT,
                                          @donGia DECIMAL(18, 2)
AS
...
CREATE PROCEDURE sp_XoaCTHoaDonDichVu @maHD NVARCHAR(10),
                                      @maDV NVARCHAR(10)
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE PROCEDURE sp_LayCTHoaDonDichVuTheoMaHD @maHD NVARCHAR(10)
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE PROCEDURE sp_TinhTongTienDichVu @maHD NVARCHAR(10)
AS
BEGIN
    SET NOCOUNT ON;
...
BEGIN
    ...
    WHERE
      ...
      -- tăng cường khả năng tìm kiếm chính xác
        maHD = LTRIM(RTRIM(@MaHD))
      AND maDV = LTRIM(RTRIM(@MaDV));
END
GO
-- Cập nhật số lượng dịch vụ trong CTHoaDonDichVu
CREATE PROCEDURE sp_CapNhatSoLuongCTHDDV @MaHD NVARCHAR(10),
                                         @MaDV NVARCHAR(10),
                                         @SoLuongMoi INT
AS
BEGIN
