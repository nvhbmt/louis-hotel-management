...
CREATE OR ALTER PROCEDURE sp_ThemNhanVien @maNV NVARCHAR(10), -- Mã nhân viên
...
CREATE OR ALTER PROCEDURE sp_CapNhatNhanVien @hoTen NVARCHAR(100), -- Họ tên nhân viên
    @soDT NVARCHAR(15), -- Số điện thoại
    @diaChi NVARCHAR(200), -- Địa chỉ
    @chucVu NVARCHAR(50), -- Chức vụ
    @ngaySinh DATE, -- Ngày sinh
...
-- 4. Xóa nhân viên
CREATE OR ALTER PROCEDURE sp_XoaNhanVien @maNV NVARCHAR(10) -- Mã nhân viên cần xóa
    AS
BEGIN
DELETE
FROM NhanVien
...
CREATE OR ALTER PROCEDURE sp_TimNhanVienTheoMa @maNV NVARCHAR(10) -- Mã nhân viên cần tìm
    AS
BEGIN
SELECT *
FROM NhanVien
